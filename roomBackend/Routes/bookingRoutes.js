...
router.post("/book", async (req, res) => {
  try {
    ...
    if (!studentId || !propertyId) {
      return res.status(400).json({ message: "Missing studentId or propertyId" });
    }
    ...
    const newBooking = await Booking.create({ studentId, propertyId });

    // mark property as booked
    property.booked = true;
    property.bookedBy = studentId;
    await property.save();

    res.status(201).json({ message: "Property booked successfully", newBooking });
  } catch (err) {
