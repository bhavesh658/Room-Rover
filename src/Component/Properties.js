...
import sampleData from "../Data"; // Sample data for testing
...
export default function Properties() {
  ...
  const handleClick = async (type) => {
    ...
    try {
      ...
      //   `http://localhost:5000/api/properties/${type}`
      // );
      setProperties(sampleData.filter((prop) => prop.type === type));
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      {!loading && !error && (
        <>
          {selectedType && (
            <p className="mt-2 px-4 py-2 text-black rounded-lg font-bold text-3xl ">
              Showing results for: {selectedType.toUpperCase()}
            </p>
          )}
