...
export default function Properties() {
  ...
  const handleClick = async (type) => {
    ...
    setError("");
    try {
      const res = await axios.get(`http://localhost:5000/api/properties/${type}`);
  ...
  return (
    <section id='prop' className="py-16 bg-white w-[90%] md:w-[80%] lg:w-[70%] mx-auto text-center">
      ...
      <div className="flex justify-center items-center mb-8">
        <ul className="flex flex-row items-center gap-6">
          {propertyTypes.map((item) => (
            ...
            >
              <img src={item.image} alt={item.name} className="w-60 h-44 object-cover rounded-lg shadow-md" />
      ...
      {/* Loading State */}
      {loading && <p className="text-gray-500 text-lg animate-pulse">Loading properties...</p>}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {properties.length > 0 ? (
            properties.map((prop, index) => (
              <div key={index} className="bg-white rounded-xl shadow-lg p-4 text-left">
                <img src={prop.image} alt={prop.name} className="w-full h-48 object-cover rounded-lg mb-3" />
                <h3 className="text-xl font-bold text-gray-800">{prop.name}</h3>
                <p className="text-gray-600">{prop.location}</p>
                <p className="text-yellow-600 font-semibold">₹{prop.rent} / month</p>
                <p className="text-sm text-gray-500 mb-2">Owner: {prop.owner}</p>
            ...
            <p className="text-gray-500">No properties found for {selectedType}</p>
          ) : (
            <p className="text-gray-400">Click on a category to view properties</p>
          )}
        </div>
      )}
    </section>
